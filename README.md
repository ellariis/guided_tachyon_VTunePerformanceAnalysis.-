...
make VERBOSE=1
```
3. To run an already created executable without the Makefile, use the following command: `./<name of executable> ../dat/balls.dat`. For example, in order to run the `tachyon.openmp` executable from the `linux` directory of this project folder, use this command: `./tachyon.openmp ./dat/balls.dat`.
4. To open the VTune GUI from the Linux command line, use the command `vtune-gui`. 
5. Select `Configure Analysis`. 
6. In the `Launch Application` section, ensure that you provided the right path to your executable in the `Application` field, and put `../dat/balls.dat` in the `Application parameters` field. ![image](https://user-images.githubusercontent.com/111458217/189770535-068efa3f-88e6-43ce-9aef-e165e2126664.png)
7. Click the arrow next to `Hotspots` to choose what type of analysis you want to run. This tutorial will use the `Hotspots` and `Threading` analyses. </br> ![image](https://user-images.githubusercontent.com/111458217/189770790-d9c3932b-ca4d-4b2c-8105-481533a31f72.png)
8. Click the leftmost blue play button to run the analysis. </br> ![image](https://user-images.githubusercontent.com/111458217/189770834-2c772ed8-b356-4eef-bfe0-d2dacb68ec95.png)
...
### Serial Build
1. If using Visual Studio, set `build_serial` as your startup project and open VTune. If using Linux, run `vtune-gui` to open the VTune GUI and provide the path to `tachyon.serial` in the `Application` field.
...
2. Repeat step 1 in the Serial Build section, except set `build_with_openmp` as your startup project instead. If using Linux, provide the path to `tachyon.openmp` in the `Application` field.
...
2. Repeat step 1 in the Serial Build section, except set `build_with_tbb` as your startup project instead. If using Linux, provide the path to `tachyon.tbb` in the `Application` field.
